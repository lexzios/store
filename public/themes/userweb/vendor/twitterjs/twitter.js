...
var twitterFeed = {
	...
	options: {
		...
		count: 2,
		total_request: 3,
		URL: "/themes/userweb/php/twitter-feed.php",
		wrapper: "tweet"
	},
	...
	initialize: function( options ) {
		...
		this.options.URL = this.options.URL + "?twitteruser=" + this.options.id + "&notweets=" + this.options.total_request;
		this.build();
			...
			function (feeds) {
				...
				for (var i = 0; i < feeds.length; i++) {
					...
					if (((showretweets == true) || ((isaretweet == false) && (showretweets == false))) && ((showdirecttweets == true) || ((showdirecttweets == false) && (isdirect == false)))) {
						if ((feeds[i].text.length > 1) && (displayCounter <= $this.options.total_request)) {
